...
function editProfile(){
  ...
  const birthDate=state.user.birth_date||state.session?.user?.user_metadata?.birth_date||'';
  modal(`<h2>Редактирование профиля</h2><form class="form" onsubmit="saveProfile(event)"><div class="field"><label>ФИО</label><input name="name" required minlength="5" autocomplete="name" value="${escapeHtml(currentName)}" placeholder="Фамилия Имя Отчество"></div><div class="field"><label>Дата рождения</label><input name="birth_date" type="date" required value="${escapeHtml(birthDate)}"></div><div class="field"><label>Город</label><input name="city" required minlength="2" value="${escapeHtml(state.user.city||'Москва')}"></div><div id="profileError" class="authError" hidden></div><button class="btn primary">Сохранить</button></form>`)
}
function validateProfileForm(form){
  ...
  if(!d.name||d.name.split(' ').filter(Boolean).length<2)return fail('Укажите полное ФИО — минимум фамилию и имя.');
  if(!d.birth_date)return fail('Укажите дату рождения.');
...
function verifyIdentity(){modal(`<h2>Подтверждение личности</h2><p class="muted">Паспорт проверяется отдельно для каждого пользователя. Профессиональный статус подтверждается на следующем шаге.</p><div class="field"><label>Документ</label><select><option>Паспорт РФ</option><option>Иностранный паспорт</option><option>Вид на жительство</option></select></div><div class="field" style="margin-top:12px"><label>Разворот с фотографией</label><input type="file" accept="image/*,.pdf" required></div><div class="field" style="margin-top:12px"><label>Страница с регистрацией</label><input type="file" accept="image/*,.pdf"></div><button class="btn primary" style="margin-top:16px" onclick="state.user.identity_verified=true;state.user.verified=true;state.trustScore=Math.max(state.trustScore,40);save();closeModal();render();toast('Документы отправлены на проверку')">Отправить на проверку</button>`)}
function verifyProfessionalStatus(){modal(`<h2>Подтверждение статуса</h2><p class="muted">Этот шаг нужен исполнителям и организациям. Проверка паспорта проводится отдельно.</p><div class="field"><label>Статус</label><select id="legalStatus"><option>Самозанятый</option><option>ИП</option><option>ООО</option><option>АО</option><option>НКО</option><option>Другая организация</option></select></div><div class="field" style="margin-top:12px"><label>Подтверждающий документ</label><input type="file" accept="image/*,.pdf"></div><button class="btn primary" style="margin-top:16px" onclick="state.user.legal_verified=true;state.trustScore=Math.max(state.trustScore,55);save();closeModal();render();toast('Статус отправлен на проверку')">Отправить на проверку</button>`)}
function verifyProfile(){verifyIdentity()}
function showAccountSettings(){modal(`<h2>Настройки аккаунта</h2><div class="settingsList"><label class="settingRow"><span><b>Уведомления</b><small>Сообщения, отклики и обновления</small></span><input type="checkbox" checked></label><label class="settingRow"><span><b>Показывать статус онлайн</b><small>Другие пользователи увидят, что вы в сети</small></span><input type="checkbox" checked></label><button class="settingsAction" onclick="showLegal('privacy')"><span><b>Конфиденциальность</b><small>Как обрабатываются ваши данные</small></span><i>›</i></button><button class="settingsAction" onclick="toast('Смена пароля будет отправлена на email')"><span><b>Сменить пароль</b><small>Получить ссылку на почту</small></span><i>›</i></button></div><button class="btn danger settingsLogout" onclick="closeModal();signOut()">Выйти из аккаунта</button>`)}
function showSupport(){modal(`<h2>Поддержка</h2><p class="muted">Опишите вопрос — обращение будет сохранено для команды поддержки.</p><form class="form" onsubmit="event.preventDefault();closeModal();toast('Обращение отправлено')"><div class="field"><label>Тема</label><select><option>Аккаунт и вход</option><option>Объявления</option><option>Оплата и бонусы</option><option>Безопасность</option><option>Другое</option></select></div><div class="field"><label>Сообщение</label><textarea required minlength="10" placeholder="Опишите проблему подробнее"></textarea></div><button class="btn primary">Отправить обращение</button></form>`)}
...
function showBonusShop(){modal(`<h2>💎 Помогай Бонусы</h2><p class="muted">Баланс: <b>${state.bonusBalance} бонусов</b></p><div class="bonusRate">Оплата бонусами: <b>1,5 бонуса = 1 ₽</b></div><div class="bonusGrid bonusGridSingle">${bonusPacks.map(([r,b],i)=>`<button class="bonusPack ${i===2?'best':''}" onclick="buyBonusDemo(${b})"><span><strong>${b} бонусов</strong><small>${r.toLocaleString('ru-RU')} ₽</small></span>${i===2?'<em>Выгодно</em>':''}<i>Купить</i></button>`).join('')}</div>`) }
...
async function ensureProfile(){
  ...
  if(data){
    state.user={...state.user,name:data.name||registeredName||u.email?.split('@')[0]||'Пользователь',city:data.city||u.user_metadata?.city||'Москва',birth_date:state.user.birth_date||u.user_metadata?.birth_date||'',verified:!!data.verified||!!state.user.verified,identity_verified:!!state.user.identity_verified,legal_verified:!!state.user.legal_verified,avatar_url:data.avatar_url||null};
...
saveProfile=async function(e){e.preventDefault();const d=validateProfileForm(e.currentTarget);if(!d)return;if(!state.session){state.user={...state.user,...d};save();closeModal();render();toast('Профиль обновлён');return}const r=await sb.from('profiles').update({name:d.name,city:d.city,updated_at:new Date().toISOString()}).eq('id',state.session.user.id);if(r.error)return toast(r.error.message);await sb.auth.updateUser({data:{name:d.name,city:d.city,birth_date:d.birth_date}});state.user={...state.user,...d};save();closeModal();render();toast('Профиль обновлён')};

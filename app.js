...
const savedTrustScore=store('pm_trust_score',0);
const savedUser=store('pm_user',{name:'',city:'Москва',verified:false});
// Удаляем только устаревшие демонстрационные значения из прежних сборок.
if(savedUser.name==='Светлана')savedUser.name='';
let state={session:null,cloudReady:false,page:'home',bonusBalance:store('pm_bonus_balance',0),trustScore:savedTrustScore===42?0:savedTrustScore,filter:'Все',query:'',createMode:'task',view:'list',radius:store('pm_radius',5),locationName:store('pm_location','Москва'),role:store('pm_role','customer'),fav:store('pm_fav',[]),services:store('pm_services',seed),tasks:store('pm_tasks',[]),messages:store('pm_messages',[]),user:savedUser};
const save=()=>{localStorage.setItem('pm_bonus_balance',JSON.stringify(state.bonusBalance));localStorage.setItem('pm_trust_score',JSON.stringify(state.trustScore));localStorage.setItem('pm_role',JSON.stringify(state.role));localStorage.setItem('pm_radius',JSON.stringify(state.radius));localStorage.setItem('pm_location',JSON.stringify(state.locationName));localStorage.setItem('pm_fav',JSON.stringify(state.fav));localStorage.setItem('pm_services',JSON.stringify(state.services));localStorage.setItem('pm_tasks',JSON.stringify(state.tasks));localStorage.setItem('pm_messages',JSON.stringify(state.messages));localStorage.setItem('pm_user',JSON.stringify(state.user))};
...
function profile(){
  if(!state.session){
    shell(`<section class="panel guestProfile"><div class="welcomeAvatar"><span class="personGlyph">♙</span></div><h2>Личный кабинет</h2><p class="muted">Войдите или создайте аккаунт, чтобы управлять объявлениями, чатами, бонусами и проверкой профиля.</p><div class="authChoice"><button class="btn primary" onclick="showAuth('login')">Войти</button><button class="btn outline" onclick="showAuth('signup')">Создать аккаунт</button></div></section>`);
    return;
  }
  ...
  shell(`<section class="panel profileSummary">
    <div class="profileIdentity"><div><h2>${escapeHtml(displayName)}</h2><div class="muted">${escapeHtml(state.user.city||'Москва')}</div><div class="badges" style="margin-top:8px"><span class="badge trustBadge">🛡️ ${trustLabel()}</span>${state.user.verified?'<span class="badge verifiedBadge">✓ Документы проверены</span>':'<span class="badge neutral">Документы не проверены</span>'}</div></div></div>
    <div class="trustMeter"><div class="trustMeterHead"><b>Уровень доверия</b><strong>${state.trustScore}%</strong></div><div class="trustTrack"><i style="width:${state.trustScore}%"></i></div><small>Подтвердите телефон, email, личность и статус, чтобы повысить доверие.</small></div>
    <div class="stats"><div class="stat"><b>${state.tasks.length}</b><span class="muted">публикаций</span></div><div class="stat"><b>${f.length}</b><span class="muted">в избранном</span></div><div class="stat"><b>${state.bonusBalance}</b><span class="muted">бонусов</span></div></div>
    <div class="profilePrimaryAction"><button class="btn primary" onclick="editProfile()">Редактировать профиль</button></div>
  </section>
  <section class="section panel profileQuick"><h2>Аккаунт</h2><div class="list" style="margin-top:14px">
    <div class="listItem" onclick="document.getElementById('favoritesSection')?.scrollIntoView({behavior:'smooth'})"><div class="thumb">❤️</div><div><b>Избранное</b><div class="muted">Сохранено: ${f.length}</div></div><div class="spacer"></div>›</div>
    <div class="listItem" onclick="showPlans()"><div class="thumb">⭐</div><div><b>Тарифы</b><div class="muted">Бесплатно, PRO, Business, Premium и Inclusive</div></div><div class="spacer"></div>›</div>
    <div class="listItem" onclick="showBonusShop()"><div class="thumb">💎</div><div><b>Помогай Бонусы</b><div class="muted">Баланс: ${state.bonusBalance} бонусов</div></div><div class="spacer"></div>›</div>
    <div class="listItem" onclick="go('create')"><div class="thumb">📢</div><div><b>Мои объявления</b><div class="muted">Публикации и новые размещения</div></div><div class="spacer"></div>›</div>
    <div class="listItem" onclick="go('chats')"><div class="thumb">💬</div><div><b>Чаты</b><div class="muted">Переписки и договорённости</div></div><div class="spacer"></div>›</div>
  </div></section>
  <section class="section" id="favoritesSection"><div class="sectionHead"><h2>Избранное</h2><span class="muted">${f.length}</span></div>${cards(f)}</section>
  <section class="section panel"><h2>Доверие и проверка</h2><div class="list" style="margin-top:14px">
    <div class="listItem" onclick="verifyProfile()"><div class="thumb">🪪</div><div><b>Проверить личность и документы</b><div class="muted">Физлицо, самозанятый, ИП или компания</div></div><div class="spacer"></div>›</div>
    <div class="listItem" onclick="showTrustLevels()"><div class="thumb">🛡️</div><div><b>Уровни доверия</b><div class="muted">Как получить статус надёжного исполнителя</div></div><div class="spacer"></div>›</div>
  </div></section>
  <section class="section panel"><h2>Продвижение</h2><div class="list" style="margin-top:14px"><div class="listItem" onclick="showPromotion()"><div class="thumb">🚀</div><div><b>Продвижение объявлений</b><div class="muted">Поднять, выделить или закрепить</div></div><div class="spacer"></div>›</div></div></section>
  <section class="section panel"><h2>Настройки</h2><div class="list" style="margin-top:14px">
    <div class="listItem" onclick="toast('Настройки будут добавлены в следующем релизе')"><div class="thumb">⚙️</div><div><b>Настройки аккаунта</b><div class="muted">Уведомления, безопасность и приватность</div></div><div class="spacer"></div>›</div>
    <div class="listItem" onclick="toast('Раздел поддержки будет подключён позже')"><div class="thumb">❓</div><div><b>Поддержка</b><div class="muted">Помощь по работе сервиса</div></div><div class="spacer"></div>›</div>
  </div></section>
  <section class="section profileLogout"><button class="btn danger" onclick="signOut()">Выйти из аккаунта</button></section>`)
}
...
function showBonusShop(){modal(`<h2>💎 Помогай Бонусы</h2><p class="muted">Ваш баланс: <b>${state.bonusBalance} бонусов</b>.</p><div class="bonusRate">Коэффициент оплаты бонусами: <b>1,5× к 1 ₽</b></div><div class="bonusGrid">${bonusPacks.map(([r,b],i)=>`<button class="bonusPack ${i===2?'best':''}" onclick="buyBonusDemo(${b})"><span>${b} бонусов</span><b>${r.toLocaleString('ru-RU')} ₽</b>${i===2?'<em>Выгодно</em>':''}</button>`).join('')}</div>`) }
...
function showPromotion(){modal(`<h2>🚀 Продвижение</h2><p class="muted">Оплачивайте рублями или бонусами.</p><div class="bonusRate">Коэффициент оплаты бонусами: <b>1,5× к 1 ₽</b></div><div class="promoList">${promotionItems.map(([n,r])=>`<div class="promoRow"><div><b>${n}</b><small>${Math.ceil(r*1.5)} бонусов</small></div><button class="btn outline" onclick="payPromotion(${r},'${n.replace(/'/g,"\'")}')">${r} ₽</button></div>`).join('')}</div>`) }
...
async function authSubmit(e,mode){
  ...
  try{
    ...
    const friendly=raw.includes('Invalid login credentials')?'Неверный email или пароль.':raw.includes('User already registered')?'Такой email уже зарегистрирован.':raw.includes('Email not confirmed')?'Сначала подтвердите email по ссылке из письма.':raw;
    showError(friendly);
...
async function ensureProfile(){
  ...
  if(!data){
    const row={id:u.id,name:registeredName||u.email?.split('@')[0]||'Пользователь',city:u.user_metadata?.city||'Москва',role:'customer'};
    ...
    const r=await sb.from('profiles').update({name:registeredName,updated_at:new Date().toISOString()}).eq('id',u.id).select().single();
    if(!r.error)data=r.data;
  }
  if(data){
    state.user={name:data.name||registeredName||u.email?.split('@')[0]||'Пользователь',city:data.city||'Москва',verified:!!data.verified,avatar_url:data.avatar_url||null};
    if(!state.user.verified&&state.trustScore===42)state.trustScore=0;
    save();
  }
}
